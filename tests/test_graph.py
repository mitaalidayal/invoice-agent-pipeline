...
def test_pending_review_decision_routes_to_hold_not_payment_or_reject(monkeypatch):
    monkeypatch.setattr(
        "nodes.run_approval",
        lambda extracted, validation: {
            "decision": "pending_review", "reasoning": "test", "reflection_count": 1, "flags": ["high_value"],
        },
    )
    graph = build_graph()
    result = graph.invoke(_initial_state("data/invoices/invoice_1005.json"))
    ran_nodes = [entry["node"] for entry in result["log"]]
    assert "hold_for_review" in ran_nodes
    assert "payment" not in ran_nodes
    assert "reject" not in ran_nodes
    assert result["payment_result"] is None
    assert result["approval_decision"] == "pending_review"
